...
void proc() {
    ...
    for (const auto& i : q) {
        for (const auto& j : i.nd) {
            for (const auto& k : q[i.st + 1]) {
                bool ans = true;
                for (int l = 0; l < j.nd.size(); l++) {
                    if (j.nd[l] != k.nd[l + 1]) {
                        ans = false;
                        break;
                    }
    ...
    for (int i = 1; i < k; i++) {
        // cout << "(" << par[i] << ", " << cld[par[i]].size() << ")" << endl;
        if (par[i]) {
            if (cld[par[i]].size() == 1) {
                for (const int& j : cld[i]) {
                    par[j] = 0;
                }
            } else {
                for (const int& j : cld[par[i]]) {
                    par[j] = 0;
                }
            }
            ans--;
