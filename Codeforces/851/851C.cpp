...
#define t5 tup<int, int, int, int, int>
#define t(i) get<i>(rec[p]) * get<i>(rec[p])
#define k(i) get<i>(rec[p]) * get<i>(rec[q])

int n, x, y, z, w, v, c;
vct<t5> rec;

inline long double vab(int p) {
    return __builtin_sqrtl(t(0) + t(1) + t(2) + t(3) + t(4));
}

inline long double mul(int p, int q) {
    return __builtin_sqrtl(k(0) + k(1) + k(2) + k(3) + k(4));
}

inline bool proc(int p, int q) {
    return __builtin_acosl(mul(p, q) / (vab(p) * vab(q))) < 90;
}
