...
long double dis(int x, int y, int p, int q) {
    return __builtin_sqrtl(sqr(x - p) + sqr(y - q));
}
...
int main() {
    ...
    atexit(pause);
    #endif
    get(ax); get(ay); get(bx); get(by); get(cx); get(cy);
    a[0] = dis(ax, ay, bx, by);
    a[1] = dis(ax, ay, cx, cy);
    a[2] = dis(bx, by, cx, cy);
    sort(a, a + 3);
    cout << ((a[0] + a[1] > a[2]) && (dis(ax, ay, bx, by) == dis(bx, by, cx, cy)) ? "Yes" : "No");
} 

// long long转为int之后的中间运算结果溢出
// 返回值类型错误导致的返回值溢出
// double精度不够导致的计算错误
// long double推荐使用__builtin_sqrtll
// (BOLD)对于不需进行二次运算的需开方的值可选择不开方