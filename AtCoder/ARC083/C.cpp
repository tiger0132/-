...
bool wat[32] {1}, sug[3072] {1};
tup<long double, int, int> out;
int a, b, c, d, e, f;
...
int main() {
    ...
    for (int i = a; i < 32; i++) wat[i] |= (wat[i - a] || wat[i - b]);
    for (int i = c; i < 3072; i++) sug[i] |= (sug[i - c] || sug[i - d]);
    for (int i = 1; i < 32; i++) {
        for (int j = 0; j < 3072; j++) {
            if (wat[i] && sug[j] &&
                i * 100 + j <= f && j <= i * e &&
                (100. * j) / (i * 100. + j) >= get<0>(out)) {
                out = make_tuple((100 * j) / (i * 100. + j), i * 100 + j, j);
            }
        }
    }
    put(get<1>(out)); spc; put(get<2>(out));
} 